...
% path(state(3,3,right), state(0,0,_), [], Path)
% Path = [move(0,1,Left), move(0,1,right),move(1,1,left),

% rotateLeft(+List, +Num, -Result)
% it is true if Result unify with a list with the same
% elements than List but not rotating Num times to the left
...
%
% it is true if a graphic representation of Path as list
% of movements are shown in Window

moveGraphic(Window, state(MisStateList, CanStateList), 
  [move(Mis, Can, Left)|PathTail], ElementsList) :-
